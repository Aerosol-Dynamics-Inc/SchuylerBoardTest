...
static u8 ilog2(unsigned long v)
{
	u8 l = 0;
	while ((1UL << l) < v)
		l++;
...
static bool spi_nand_scan_id_table(struct spi_nand_chip *chip, u8 *id)
{
	...
	uint64_t mySize;
	uint32_t myBlockSize;

	for (; type->name; type++) {
		if (id[0] == type->mfr_id && id[1] == type->dev_id) {
			chip->name = type->name;
			mySize = (uint64_t)(((uint64_t)(type->page_size)) * ((uint64_t)(type->pages_per_blk)) * ((uint64_t)(type->blks_per_lun)) * ((uint64_t)(type->luns_per_chip)));
			chip->size = mySize;
			myBlockSize = (uint32_t)(((uint32_t)type->page_size) * ((uint32_t)type->pages_per_blk));
			chip->block_size = myBlockSize;
